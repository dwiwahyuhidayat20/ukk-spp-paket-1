...
class Data_model extends CI_Model
{
    ...
    public function cariDataSiswa()
    {
        ...
        $query = $this->db->query("SELECT `tbl_siswa`.*, `tbl_kelas`.`nama_kelas`, `tbl_spp`.`tahun`
        FROM `tbl_siswa` JOIN `tbl_kelas`
        ON `tbl_siswa`.`id_kelas` = `tbl_kelas`.`id_kelas`
        JOIN `tbl_spp` ON `tbl_siswa` .`id_spp` = `tbl_spp`.`id_spp` WHERE `tbl_siswa`.`NAMA` LIKE '%" . $keyword . "%' ORDER BY `tbl_siswa`.`NISN`, `tbl_siswa`.`id_kelas` ASC");

        // mysqli_next_reuslt($this->db->conn_id);
        return $query->result();
    }
